...
public class JavaFileUtils {
	...
	/**
	 * 根据字段信息 生成java属性信息。如 varchar username-->private String uername;以及相应的set get方法源码
	 * @param column	字段信息
...
//		System.out.println(src);
	...
	public static void createJavaPOFile(TableInfo tableInfo,TypeConvertor convertor) {
		...
		//正则转为java每个\2,故popackage里面的.换成\\是（正则.特殊符号本身需要加\）"\\."-->"\\\\"
		...
		try {
			...
			System.out.println("建立表"+tableInfo.getTname()+
					"对应的的java类： "+StringUtils.firstChar2UpperCase(tableInfo.getTname())+".java");
		} catch (IOException e) {
			e.printStackTrace();
		}finally {
			try {
...
//		System.out.println(f);
	...
	//因为初始化TableContext的时候加载了静态类，里面有该方法的updataJavaPOFile(),所以会显示两遍“建立表。。。对应的。。。。类：。。。”
		Map<String,TableInfo> map = TableContext.tables;	
//		TableInfo t = map.get("emp");
//		String str = createJavaSrc(t, new MySqlTypeConvertor());
